...
export async function performNavigation(
  ...
  // Installed as the LAST statement before the try, and it has to stay that way. These
  // are two page listeners whose only removal is the watch.stop() in the finally below,
  // so anything that could throw between installing them and entering the try strands
  // them on the page for the rest of its life, holding a closure over an activity array
  // that grows with every document response. The snapshot read below used to sit in that
  // gap. It is inside the try now, which changes nothing about when it runs relative to
  // the navigation and everything about what happens when it fails.
  ...
  try {
    ...
    before = await readPageSnapshot(page, Math.min(SNAPSHOT_READ_TIMEOUT_MS, remaining()));
    try {
