...
(small and known upfront), this was a better trade than building and
...
The suite (298 tests as of v1.2), `npm test` (builds, then runs against the real compiled
...
## 10. v1.2: what a six-agent battle-test found, and the surface it forced

v1.1 was written from the inside. v1.2 is written from evidence: six QA
agents drove seven concurrent isolated sessions through roughly 470 tool
calls against a live React app. The core claim held perfectly. Zero tool
failures, genuine isolation, no cross-talk, no crashes. Everything below
is what got in the way, and the surface grew from 15 tools to 58.

### 10.1 The governing rule this round established

**A tool must report what actually happened, never what it was asked to
do.** Two bugs made this the organising principle rather than a nicety.

`navigate` to a URL differing only in the hash returned a payload
identical to a real page load while the JS context, React state and
console buffer all survived. Four of five agents hit it; two recorded
false passes and had to discard finished work. Any hash-routed SPA makes
this the normal case, not an edge case. One agent's summary is the rule in
one line: *a success payload identical to a real load is the trap.*

`fill` on a CodeMirror surface inserted instead of replacing, silently
producing a concatenation. That is worse than a visible failure: it
corrupts the test's own input, so the agent reasons about a value it never
set.

The general fix is not the two specific patches. It is that a tool which
writes reads the value back and reports what it really is; a tool that
cannot do what was asked says so; and a tool that did something subtly
different (a same-document navigation, a drag the browser converted into a
text selection, a route rule that never matched, a wheel that scrolled
nothing) says that explicitly. Building this out found several more
instances of the same shape that nobody had reported, which is the
argument for treating it as a rule rather than a bug list.

### 10.2 Lifecycle defects, and why they were the dangerous ones

**The daemon ignored live sessions when deciding to exit.** The gate was
"client registry is empty", so the moment every client wrapper exited or
was pruned as dead, the daemon took the shared Chromium down and killed
every in-flight session. Client wrappers and browser sessions have
genuinely independent lifetimes: a wrapper can be killed while the work it
started is still running. The gate now reads both counts. The daemon still
terminates on its own, because orphaned sessions go idle and are reaped by
the same pass, which then finds both counts at zero.

**A client wrapper never recovered from a dead daemon.** Confirmed by
probe: kill the daemon and every later call returns `fetch failed`
forever, because the wrapper memoized its connected client and only
cleared it when the initial connect rejected. This is the defect that made
the whole design's "the daemon comes and goes on demand" premise false in
practice: either a self-shutdown or a restart-to-pick-up-a-fix bricked
every open session. A connection-level failure now drops the client,
respawns the daemon and retries once, with a deliberately narrow
classifier so work that really ran is never retried.

**Reaping raced its own callers.** `lastActivity` was touched when a call
started and never while it ran, so anything past the idle timeout lost its
context mid-call. `escalate_session` was sharper still: a human driving a
session makes no tool calls, so the tool's own scenario reaped itself
after fifteen minutes. Sessions now carry an in-flight count and an
escalated flag with a longer timeout. That veto then needed its own bound,
since a never-settling call could pin a session and through it the shared
daemon, so a call past `HARBORAGE_MAX_IN_FLIGHT_AGE_MS` is treated as
stuck.

Three mechanisms layered on one timer is the most intricate part of the
system, and it is deliberately all in one sweep pass with documented
ordering rather than spread across several.

### 10.3 Observability was the quiet failure

Over an entire working evening the daemon log contained only alternating
`daemon listening` and `shutting down` lines, with no timestamps and no
session events. When a subagent reported a dead session there was nothing
to diagnose from, which is why finding the shutdown bug took a code read
rather than a log read. The log now carries one timestamped structured
line per event, including every shutdown decision *and the ones that
decline*, so the trigger is visible after the fact.

### 10.4 On tool count

15 to 58 is a large jump, and the cost is real: every description is
loaded into every agent's context. It was accepted because the alternative
is worse. Each missing primitive was not merely inconvenient, it pushed
agents into hand-rolled workarounds through `evaluate` and
`send_cdp_command` that were *less correct in ways they could not see*.
Synthetic hover events test the listeners rather than the browser and
cannot detect a CSS-only `:hover`. Programmatic focus does not set
`:focus-visible`, so an accessibility pass reports no focus ring anywhere
and reads as a clean sweep. A CDP viewport override changes `innerWidth`
while screenshots keep capturing at the old size. In each case the
workaround produced a confident wrong answer, so the tool is not a
convenience, it is the difference between a true and a false report.

The same logic sets the boundary. Three things Playwright MCP exposes were
deliberately not built, because the existing surface already covers them
without a correctness gap: issuing a request (`evaluate` with `fetch`,
which unlike an external HTTP client runs on the page's own origin and
cookie jar), filling a form field by field (`fill` in a loop), and
tracing or video capture (nothing asked for it, and screenshots plus the
console and network buffers covered every real investigation).

### 10.5 Isolation held, and was re-proven

Nothing in this round weakened it. That mattered more than usual because
much of the new surface mutates context-level rather than page-level
state: route rules, permissions, geolocation, the fake clock, offline
mode, cookies and storage. Each of those is a fresh chance for one
session's state to reach another, so each stream that added one was
required to prove non-interference between concurrent sessions, and the
whole surface was then re-attacked by adversarial reviewers who had not
written any of it.
